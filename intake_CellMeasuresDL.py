...
if __name__ == "__main__":
    ...
    ####### TEST FUNCTION
    # Complete - TODO add check to see if downloadable DF has already been exported.
    ...
    if not os.path.isfile(filename):
        ...
        u_response =input(f"Type 'new' if you want a new catalogue search for {var}.\nAlternatively, type 'skip' to trigger downloads using existing file {filename}:")
        if u_response.lower().strip(" ") == 'new':
            varcell_prepare_df(logger_object=logger1, variable_id=var)


    #### SKIP IF DOWNLOADABLE HAS BEEN EXPORTED ####
    #
    # ########################
    # # scrape to see which ones to include
    # cat = ESGFCatalog().search(
    ...
    user_input = input("Do you want to continue to trigger downloads? (yes/no): ")
